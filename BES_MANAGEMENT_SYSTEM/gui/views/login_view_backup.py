...
class LoginWindow(QtWidgets.QDialog):
    ...
    def handle_login(self):
        ...
        if not username or not password:
            self.notification.show_warning("⚠️ Please enter both username and password")
            return
        ...
        QtCore.QTimer.singleShot(1500, self.open_sidebar_home)
    
    def open_sidebar_home(self):
        """Open the sidebar home window"""
        try:
            from gui.views.sidebar_home_view import SidebarHomeWindow
            self.sidebar_home = SidebarHomeWindow()
            self.sidebar_home.show()
            self.close()
        except Exception as e:
            self.notification.show_error(f"Error opening home: {e}")
